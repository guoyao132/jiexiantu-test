 ...
 */
mxGraphView.prototype.validateCellState = function (cell, recurse, isLight) {
  recurse = (recurse != null) ? recurse : true;
  var state = null;
  ...
  if (cell != null) {
    ...
    if (state != null) {
      ...
      if (state.invalid) {
        ...
        if(isLight){
          let id = cell?.id || '';
          state.style.fontColor = graphThemeColor.defaultFontColorToLightColor(state.style.fontColor, id);
        }
        if (cell != this.currentRoot) {
          this.validateCellState(model.getParent(cell), false, isLight);
        }

        state.setVisibleTerminalState(this.validateCellState(this.getVisibleTerminal(cell, true), false, isLight), true);
        state.setVisibleTerminalState(this.validateCellState(this.getVisibleTerminal(cell, false), false, isLight), false);
      ...
      if (recurse && !state.invalid) {
        ...
        for (var i = 0; i < childCount; i++) {
          this.validateCellState(model.getChildAt(cell, i), recurse, isLight);
        }
      }
    }
  }
