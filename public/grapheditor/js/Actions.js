...
Actions.prototype.init = function () {
  ...
  function deleteCells(includeEdges) {
    ...
    var canDelCells = graph.getDeletableCells(delCells);
    let lineArr = canDelCells.filter(cell => cell.id.includes('line-'));
    if (lineArr.length > 0) {
      let names = lineArr.map(cell => cell.value).join('、');
      ui.$ElMessageBox.confirm(`确认删除任务 ${names} 吗？`, '提示', {
        confirmButtonText: '确认',
        cancelButtonText: '取消',
        autofocus: false,
      }).then(() => {
        let index = 0;
        let idsStr = lineArr.map(cell => cell.id).join(',');
        let lineIds = idsStr.split(',').map((lineId) => lineId.split('-')[1]) || [];
        let ids = ui.$displayUtil.resultDate.filter((r) => lineIds.includes(r.serialNumber)).map((i) => i.id || '');
        ids.forEach(id => {
          ui.$api.removeDiagram({
            diagramId: id
          }).then(() => {
            index++;
            if (index === ids.length) {
    ...
    return
    var select = graph.deleteCells(graph.getDeletableCells(lineArr), includeEdges);
    if (select != null) {
